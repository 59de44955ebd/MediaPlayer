...
from dshow.comtypes import patcher
...
from dshow.comtypes.GUID import GUID
_GUID = GUID
IID = GUID
DWORD = c_ulong
...
def _is_object(obj):
    ...
    from dshow.comtypes.automation import VARIANT
        ...
        if has_name("_NewEnum"):
            ...
            class _(object):
                def __iter__(self):
                    ...
                    # IEnumVARIANT
                    from dshow.comtypes.automation import IEnumVARIANT
...
def CoGetClassObject(clsid, clsctx=None, pServerInfo=None, interface=None):
    if clsctx is None:
        clsctx = CLSCTX_SERVER
    if interface is None:
        import dshow.comtypes.server
        interface = dshow.comtypes.server.IClassFactory
    p = POINTER(interface)()
...
################################################################
from dshow.comtypes._comobject import COMObject
...
from dshow.comtypes._meta import _coclass_meta
