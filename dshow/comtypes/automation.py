...
from dshow.comtypes import IUnknown, GUID, IID, STDMETHOD, BSTR, COMMETHOD, COMError
...
#            self.vt = VT_ARRAY | obj._vartype_
            ...
            from dshow.comtypes.typeinfo import GetRecordInfoFromGuids
    ...
    def _get_value(self, dynamic=False):
        ...
        if vt in (VT_EMPTY, VT_NULL):
            ...
            if not dynamic:
                ...
                from dshow.comtypes.client.dynamic import Dispatch
                return Dispatch(ptr)
        # see also c:/sf/pywin32/com/win32com/src/oleargs.cpp
        elif self.vt & VT_BYREF:
            return self
        elif vt == VT_RECORD:
            from dshow.comtypes.client import GetModule
            from dshow.comtypes.typeinfo import IRecordInfo
...
class IDispatch(IUnknown):
    ...
    def GetTypeInfo(self, index, lcid=0):
        ...
        return result.QueryInterface(dshow.comtypes.typeinfo.ITypeInfo)
